<template>
  <WorkMain :headerItems="['NLP应用','情感分析']">
    <el-row>
      <el-col :span="12">
  ...
  export default {
    ...
    methods: {
      subBaseInfos:function(){
        if(this.radio=='textarea'){
          ...
          if(checkInfoNull(this.uploadFilePath,'请上传文件',this.Message)){
            ...
            var rawFile = this.uploadFile.raw
            sendDataForm.append('file',rawFile,rawFile.name);
            this.Request({
              url:'/sentiment/input/upload',
              method:'post',
              data:sendDataForm
            }).then(response=>{
              if(response.status!='0'){
      ...
      nextStep(responseData){
        console.log("next step :"+JSON.stringify(responseData))
        this.$router.push({name:"sentimentAnalyzeResult",params:{"resultData":responseData.result}})
      }
    }
  }
</script>
