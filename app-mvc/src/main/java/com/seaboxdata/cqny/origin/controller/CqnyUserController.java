...
//        Map<String,Object> selectOriginMap = new HashMap<>();
    ...
    public JsonResult getValidateCode(String user_name){
        User userInfo = userService.getUserByUserNm(user_name);
        if(userInfo!=null){
            UserForgetPwdRecord userForgetpwdRecord = cqnyUserService.getUserForgetPwdRecord(userInfo.getUser_id());
            String valiteCode = null;
            if(userForgetpwdRecord!=null){
                valiteCode = cqnyUserService.updateValidateCode(userInfo.getUser_id());
            }else{
                valiteCode = cqnyUserService.newValidateCode(userInfo.getUser_id());
            }
                ...
                if(userForgetpwdRecord==null){
                    ...
                    if(!userForgetpwdRecord.getValidate_code().equals(validate_code)){
                        ...
                        if(calendar.before(currCalendar)){
                            ...
                            Map<String, String> validateCode = cqnyUserService.getSmsValidateCode(userInfo.getUser_id(),phone_num);
    ...
    public JsonResult getAreaData(String parentId){
        List<ChinaAreaCode> allChindAreaCode = cqnyUserService.getAreaData(parentId);
    ...
    public JsonResult getOriginNature(){
        List<OriginNature> originNatureList = cqnyUserService.getOriginNature();
